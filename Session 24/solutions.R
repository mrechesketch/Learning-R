...
leadingNumber <- function(string){
    num <- extract("\\d+(?=\\w)", string)
    if ( identical(num, character(0)) ){
        return (1)
    } else{
...
coef <- function(pgMatrix) sapply( colnames(pgMatrix), function(x) leadingNumber(x) )

order <- function(pgMatrix) sum( coef(pgMatrix) )
...
orderFromName <- function(pgName){
    letterValues <- c("C" = 1, "S" = 1, "D" = 2, 
                "T" = 12, "O" = 24, "I" = 60)
    # look for the three string components
    letter <- extract("\\w(?=\\d)*", pgName)
    number <- extract("\\d+", pgName)
    subscript <- extract("s|d|v|i|h", pgName)
    # now get values
    letVal <- as.vector( letterValues[letter] )
    numVal <- if( identical(number, character(0)) ) 1 else as.integer(number)
    subVal <- if( identical(subscript, character(0)) ) 1 else 2
    # return the product of the three
    return( letVal*numVal*subVal ) 
}
...
loadTable <- function(pgName){
    ...
    function( reducibleRep ){
        ...
        reduction <- function(irrep) (1/h)*sum(coefs * reducibleRep * irrep)
        apply(pointGroup, 1, reduction) 
    }
}
